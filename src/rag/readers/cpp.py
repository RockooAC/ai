...
from src.rag.config import CHUNK_CONFIG, EMBEDDER_CONFIG
...
class CppReader(BaseReader):
    ...
    def __init__(
        ...
        embedder_model: str = EMBEDDER_CONFIG["MODELS"]["JINA"]["hf"]
    ):
        ...
        self.method_chunk_size = chunk_size * 8
        ...
        self.logger.info(f"Loading tokenizer for {embedder_model}")
        self.tokenizer = AutoTokenizer.from_pretrained(embedder_model, trust_remote_code=True)
        self.max_tokens = self.tokenizer.model_max_length
        self.safe_token_limit = int(self.max_tokens * 0.85)
        self.logger.info(f"Embedder max tokens: {self.max_tokens}, safe limit: {self.safe_token_limit}")

    def count_tokens(self, text: str) -> int:
        """Count tokens using the Jina tokenizer."""
    ...
    def process_node(self, node: TS.Node, source_code: str, filename: str) -> List[Chunk]:
        ...
        for child in node.children:
            ...
            if child.type == "class_specifier":
                ...
                class_text = source_code[child.start_byte:child.end_byte]
                class_token_count = self.count_tokens(class_text)
                
                if class_token_count < self.safe_token_limit:
                    class_chunk = Chunk(child.start_byte, child.end_byte, filename, self.language, 
                                    start_line=child.start_point[0] + 1, end_line=child.end_point[0] + 1)
                    class_chunk.set_scope(self.current_namespaces, class_name)
                    chunks.append(class_chunk)
                    self.logger.info(f"Class '{class_name}' fits in one chunk ({class_token_count} tokens)")
                else:
                    self.logger.info(f"Class '{class_name}' too large ({class_token_count} tokens), splitting")
                    class_content = self.process_node(child, source_code, filename)
                    for chunk in class_content:
                        chunk.set_scope(self.current_namespaces, class_name)
                    chunks.extend(class_content)
                ...
                current_chunk = Chunk(child.end_byte, child.end_byte, filename, self.language, start_line=child.start_point[0] + 1, end_line=child.end_point[0] + 1)
                current_chunk = Chunk(child.end_byte, child.end_byte, filename, self.language, 
                                    start_line=child.start_point[0] + 1, end_line=child.end_point[0] + 1)
                current_chunk.set_scope(self.current_namespaces, self.current_class)
                continue
