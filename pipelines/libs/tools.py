...
class CodeMultiCollectionRetriever(MultiCollectionRetriever):
    ...
    def retrieve_code_multi(
        ...
        """    
        Step 1: strict AND combos
        Example:
            If entities = {
               "class_names": ["NVCUVIDDecoder"],
               "method_names": ["process", "flush"],
               "namespaces": [],
               "file_paths": []
            }
        
           This step builds products like:
               (class="NVCUVIDDecoder" AND method="process")
               (class="NVCUVIDDecoder" AND method="flush")
        """
        if any([class_names, method_names, namespaces, file_paths]):
            for combo in itertools.product(
                class_names or [None],
                method_names or [None],
                namespaces or [None],
                file_paths or [None],
            ):
                c, m, ns, fp = combo
        ...
        """
        Step 2: fallback to separate filters
        Example:
            If entities = {
               "class_names": ["NVCUVIDDecoder"],
               "method_names": ["process", "flush"],
            ...
            Step 1 (strict AND combos) would try:
               (class="NVCUVIDDecoder" AND method="process")
               (class="NVCUVIDDecoder" AND method="flush")
            If no results are found for those strict combos,
            Step 2 falls back to separate queries:
               (class="NVCUVIDDecoder")
               (method="process")
               (method="flush")
        
            This ensures that even if metadata is incomplete we can still
            retrieve useful chunks that match at least one filter individually.
        """
        if not all_results and any([class_names, method_names, namespaces, file_paths]):
            self.logger.info("No results with AND-combos, falling back to separate filters...")

            filters = [
                ("class_name", class_names or []),
                ("method_name", method_names or []),
                ("namespace", namespaces or []),
        ...
        # Step 3: final fallback (no filters)
        if not all_results:
            self.logger.info("No results with any filters, retrying without filters...")
            all_results = self.retrieve_code(
                query, apply_filters=False, search_context=search_context, observer=observer, strict=strict,
            )
    ...
    def retrieve_code(
        ...
        self.logger.info(f"Returning {len(results)} nodes after postprocessing.")
        return results
...
class MessagesRetriever(BaseRetriever):
    ...
    def _retrieve(self, query_bundle: QueryBundle) -> List[NodeWithScore]:
        ...
        # Iteracja po parach (current, next) bez wychodzenia poza zakres
        for current_msg, next_msg in zip(self.messages, self.messages[1:]):
...
class FilterCodeCollectionRetriever(BaseRetriever):
    ...
    **kwargs: Any,
        ...
        self.logger.info(f"[search] query='{query}', apply_filters={apply_filters}, strict={strict}")  # NEW
        if observer:
        ...
        self.logger.info(f"[search] Incoming metadata filters: {kv}")

        query_str = query if query else "code"
        results: List[NodeWithScore] = []

        def run_retriever(filters: Optional[MetadataFilters] = None) -> List[NodeWithScore]:
            self.logger.info(f"[retriever] Running retriever with filters={filters}")
            retriever = self.index.as_retriever(
                filters=filters,
                similarity_top_k=self.similarity_top_k,
                retriever_mode=ListRetrieverMode.DEFAULT,
            )
            res = retriever.retrieve(query_str)
            self.logger.info(f"[retriever] Retrieved {0 if not res else len(res)} results")
            return res or []

        metadata_filters: Optional[MetadataFilters] = None

        # STEP 1: strict AND filters
        if apply_filters:
            and_filters = []
            for k, v in kv.items():
                if v is not None:
                    operator = FilterOperator.TEXT_MATCH
                    and_filters.append(MetadataFilter(key=k, operator=operator, value=v))
        
            self.logger.info(f"[search] Constructed AND filters: {and_filters}")

            if and_filters:
                metadata_filters = MetadataFilters(filters=and_filters, condition=FilterCondition.AND)
                self.logger.info(f"[search] Applying AND metadata filters: {metadata_filters}")  # existing

        if metadata_filters:
            self.logger.info("[search] Executing strict AND filter search")
            results = run_retriever(filters=metadata_filters)
            self.logger.info(f"[search] AND filter results: {len(results)}")

        # STEP 2: relaxed OR fallback
        if not strict and not results and apply_filters:
            self.logger.info("[search] No results with AND filters — evaluating OR fallback")
            or_filters = []
            for k, v in kv.items():
                if v is not None:
                    operator = FilterOperator.TEXT_MATCH # alternatively we can use FilterOperator.CONTAINS
                    or_filters.append(MetadataFilter(key=k, operator=operator, value=v))
        
            self.logger.info(f"[search] Constructed OR filters: {or_filters}")

            if or_filters:
                fallback = MetadataFilters(filters=or_filters, condition=FilterCondition.OR)
                self.logger.info(f"[search] Retrying with OR filters: {fallback}")
                results = run_retriever(filters=fallback)
                self.logger.info(f"[search] OR filter results: {len(results)}")

        # STEP 3: no filters at all
        if not strict and not results:
            self.logger.info("[search] No results with filters — retrying without filters")
            results = run_retriever(filters=None)
            self.logger.info(f"[search] Unfiltered results: {len(results)}")

        if observer:
            observer.stop(key=event_key, result_count=len(results))

        self.logger.info(f"[search] Final result count: {len(results)}")
        for i, node in enumerate(results):
            text = getattr(node, "text", None) or getattr(node.node, "text", None)
            preview = text[:300].replace("\n", " ") + ("..." if len(text) > 300 else "")
            self.logger.info(f"[search] Result[{i}] score={getattr(node, 'score', None)} text_preview='{preview}'")
...
class CodeMethodsExtractor:
    ...
    def __init__(
        ...
        base_url: str = "http://10.255.240.155:11434",  # mwa host
        temperature: float = 0.1,
    ...
    def llm_extract(self, diff: str, header: Optional[str]) -> Tuple:
        """LLM extraction"""
        ...
        try:
            ...
            identifiers = parsed.get("identifiers", []) if isinstance(parsed, dict) else []
            removed_identifiers = parsed.get("removed_identifiers", []) if isinstance(parsed, dict) else []
...
def tabify_lines(text: str) -> str:
    """
    Adds a tab character to the beginning of each line in the input string.

    Parameters:
        text (str): The input multiline string.

    Returns:
        str: The modified string with tabs at the start of each line.
    """
    return '\n'.join('\t' + line for line in text.splitlines())

def escape_curly_brackets(s: str) -> str:
    """
    Escapes all curly brackets in the input string by doubling them.
    Useful for safely using strings in str.format().

    Example:
        Input: "Hello {name}, welcome to {place}!"
        Output: "Hello {{name}}, welcome to {{place}}!"
    """
