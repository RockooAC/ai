...
class Pipeline:
    ...
    class Valves(BaseModel):
        # vLLM main settings
        VLLM_MODEL_BASE_URL: str = Field(default="http://10.13.16.51:8000/v1", description="vLLM base URL")
        VLLM_MODEL_NAME: str = Field(
            default="cpatonn/Qwen3-Next-80B-A3B-Instruct-AWQ-4bit", description="vLLM base model name"
        )
        VLLM_MAX_TOKENS: int = Field(default="92160", description="vLLM max tokens")
        VLLM_TEMPERATURE: float = Field(default="0.5", description="vLLM temperature")
        ...
        OLLAMA_EMBEDDING_BASE_URL: str = Field(
            default="http://10.255.246.131:11434", description="Ollama embedding base URL"
        )
        OLLAMA_EMBEDDING_MODEL_NAME: str = Field(
            default="gte-qwen2.5-instruct-q5", description="Ollama embedding model name"
        )
        ...
        OLLAMA_QUERIES_BASE_URL: str = Field(
            default="http://10.255.240.161:11434", description="Ollama queries base URL"
        )
        OLLAMA_QUERIES_MODEL_NAME: str = Field(default="mistral", description="Ollama queries model name")
        OLLAMA_QUERIES_STRATEGY: str = Field(
            default="diversify",
            description="Ollama queries strategy",
            examples=["diversify", "specify", "broaden", "rephrase", "diagnose"],
        )
        OLLAMA_QUERIES_SIMILARITY_THRESHOLD: float = Field(
            default=0.8, description="Ollama queries similarity threshold"
        )
        ...
        QDRANT_COLLECTION_NAME: str = Field(
            default="confluence_prod_bm42,docusaurus_prod_bm42,pdf_prod_bm42", description="QDRANT collection name"
        )
        ...
        SPARSE_TEXT_EMBEDDING_MODEL: str = Field(
            default=DEFAULT_SPARSE_TEXT_EMBEDDING_MODEL, description="Model name for sparse text embedding"
        )
        # RERANK SETTINGS
        # Similarity cutoff
        ...
        SENTENCE_TRANSFORMER_RERANK_ACTIVE: bool = Field(
            default=False, description="SENTENCE_TRANSFORMER_RERANK_ACTIVE"
        )
        SENTENCE_TRANSFORMER_RERANK_MODEL: str = Field(
            default=DEFAULT_SENTENCE_TRANSFORMER_RERANK_MODEL, description="Model name for Sentence Transformer Rerank"
        )
        ...
        FORCE_ONLY_EXTERNAL_SOURCES: bool = Field(
            default=False,
            description="Enable force only external sources. If enabled, only external sources are used for the answer.",
        )
    ...
    def load_config(self):
        ...
        try:
            ...
            """self.llm = VllmServer(
                api_url=self.valves.VLLM_MODEL_BASE_URL,
                max_new_tokens=self.valves.VLLM_MAX_TOKENS,
                temperature=self.valves.VLLM_TEMPERATURE,
            )"""

            self.llm = OpenAILike(
                model=self.valves.VLLM_MODEL_NAME,
                api_base=self.valves.VLLM_MODEL_BASE_URL,
                api_key="dummy",
                context_window=self.valves.VLLM_MAX_TOKENS,
                temperature=self.valves.VLLM_TEMPERATURE,
                is_chat_model=True,
            ...
            self.ollama_rerank = Ollama(
                ...
                context_window=self.valves.OLLAMA_RERANK_CHOICE_BATCH_SIZE * self.valves.OLLAMA_CHUNK_SIZE
                + self.valves.OLLAMA_CHUNK_OVERLAP,
                request_timeout=DEFAULT_REQUEST_TIMEOUT,
            )
    ...
    def pipe(
        ...
        if self.valves.CHAT_HISTORY_ACTIVE and len(messages) > 1:
            # MessagesRetriever powinien bezpiecznie obsługiwać listy wiadomości
            safe_messages = messages.copy()
            retrievers.append(MessagesRetriever(messages=safe_messages))
